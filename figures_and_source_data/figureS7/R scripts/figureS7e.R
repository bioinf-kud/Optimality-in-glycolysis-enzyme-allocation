...
head <- "/figureS7/data/sce.glycolysis"
tail <- ".tsv"
...
calc_rmse <- function(data, vector, label) {
  ...
  data.frame(value = vals, datanum = datanum, per = label)
}

d1 <- calc_rmse(data1, vector, "original data")
d2 <- calc_rmse(data2, vector, "a permuted")
d3 <- calc_rmse(data3, vector, "[E] permuted")
d4 <- calc_rmse(data4, vector, "both permuted")

Omean <- mean(d1$value, na.rm = TRUE)
Amean <- mean(d2$value, na.rm = TRUE)
Emean <- mean(d3$value, na.rm = TRUE)
Bmean <- mean(d4$value, na.rm = TRUE)
...
p<-ggplot(dataf) +
  ...
  labs(
    title = expression(
      atop("RMSE between "* log[10]*K* " and " * log[10]*CAQ*" (EOI) for","glycolysis enzymes in " * italic("S.cerevisiae") *" samples")
    ),
    y = "RMSE",
    col = "method"
  ) +
...
ggsave(paste0(dir,"/figureS7/figures/","figureS7e.pdf"), p, width = 650/96, height = 700/96, units = "in", device = "pdf")

...
file <- paste0(dir, "/figureS10/data/yeast_sum_RMSE.tsv")
aggrt <- read.table(file, header = TRUE, sep = "\t",)

spearman <- cor.test(aggrt$molecule.fraction.of.glycolysis.enzyme, aggrt$RMSE, method = "spearman")
rho <- round(spearman$estimate, 3)
pval <- signif(spearman$p.value, 2)

p<-ggplot(aggrt, aes(x = molecule.fraction.of.glycolysis.enzyme, y = RMSE)) +
  ...
  annotate(
    ...
    y = max(aggrt$RMSE),
    label = sprintf("atop(\"Spearman's\"~rho==%.3f, P==%.2e)", rho, pval),
    parse = TRUE,
    hjust = 0.7, vjust = 1,
    size = 7, fontface = "plain", color = "black"
  )
ggsave(paste0(dir,"/figureS10/figures/","figureS10d.pdf"), p, width = 650/96, height = 700/96, units = "in", device = "pdf")

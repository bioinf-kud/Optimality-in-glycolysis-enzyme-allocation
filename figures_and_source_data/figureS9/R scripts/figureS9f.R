...
spearman <- cor.test(aggrt$molecule.fraction.of.glycolysis.enzyme, aggrt$Corr_Pearson, method = "spearman")
rho <- round(spearman$estimate, 3)
pval <- signif(spearman$p.value, 2)

ggplot(aggrt, aes(x = molecule.fraction.of.glycolysis.enzyme, y = Corr_Pearson)) +

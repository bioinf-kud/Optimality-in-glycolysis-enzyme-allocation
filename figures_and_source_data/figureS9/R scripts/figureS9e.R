...
corr_stats <- aggrt %>%
  ...
  group_by(type) %>%
  summarise(
    rho = cor(Glyco_Genes_Sum, Corr_Pearson, method = "spearman", use = "complete.obs"),
    p = cor.test(Glyco_Genes_Sum, Corr_Pearson, method = "spearman")$p.value
  ) %>%
  mutate(
...
ggplot(aggrt, aes(x = Glyco_Genes_Sum, y = Corr_Pearson, color = Rho_label, label = Description)) +
  ...
  geom_text_repel(
    data = subset(aggrt, type == "Carbon source"),
    size = 5, hjust = 0, vjust = 1, box.padding = 0.5, max.overlaps = Inf
  ) +
  scale_color_manual(values = new_group_colors, name = expression("Group (" ~ rho ~", p-value)"))+
  labs(
    x = "Mass fraction of glycolysis enzyme",
    y = "EOI of glycolysis(Pearson)",

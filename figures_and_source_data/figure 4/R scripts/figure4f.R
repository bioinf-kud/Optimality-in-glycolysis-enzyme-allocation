...
data$K <- vector
data$pair <- c("GLCptspp-GPI", "GPI-PFK", "PFK-ALDO", "ALDO-TPI", "TPI-GAPDH", "GAPDH-PGK", "PGK-PGAM", "PGAM-ENO", "ENO-PK")
  ...
  labs(
    title = expression(
      atop(
        ...
        "enzymes in 66 "* italic("E.coli")*" samples"
      )
    ),
...
ggsave(paste0(dir,"/figure 4/figures/","figure 4f.pdf"), p, width = 650/96, height = 700/96, units = "in", device = "pdf")

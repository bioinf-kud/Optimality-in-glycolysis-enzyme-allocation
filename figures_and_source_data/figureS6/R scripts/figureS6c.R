...
data <- read.table(paste0(dir, head, "eco.glycolysis.tsv"), sep = "\t", check.names = FALSE)
data1 <- read.table(paste0(dir, head, "eco.glycolysis.no_substrate.tsv"), sep = "\t", check.names = FALSE)
...
data <- subset(data, select = -V11)
data1 <- subset(data1, select = -V11)
...
p<-ggplot(plot_df) +
  ggtitle("RMSE distribution (n=66)")+
  geom_histogram(
    aes(x = value, fill = method, colour = method),
  ...
  labs(
    title = expression(
      atop("RMSE between "* log[10]*K* " and " * log[10]*CAQ*" (EOI) for","glycolysis enzymes in 66 " * italic("E.coli") * " samples")
    ),
    x = "RMSE",
    y = "Number of samples",
    col = "method"
...
ggsave(paste0(dir,"/figureS6/figures/","figureS6c.pdf"), p, width = 650/96, height = 700/96, units = "in", device = "pdf")

...
data <- read.table(paste0(dir, head, "CCLE_proteome.glycolysis.tsv"), sep = "\t", check.names = FALSE)
data1 <- read.table(paste0(dir, head, "CCLE_proteome.glycolysis.no_substrate.tsv"), sep = "\t", check.names = FALSE)
...
p<-ggplot(plot_df) +
  ggtitle("RMSE distribution (n=378)")+
...
ggsave(paste0(dir,"/figureS6/figures/","figureS6a.pdf"), p, width = 650/96, height = 700/96, units = "in", device = "pdf")

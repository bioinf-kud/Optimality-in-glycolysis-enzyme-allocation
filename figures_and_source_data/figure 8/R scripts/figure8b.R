...
head1<-"/figure 8/data/"
head<-"/figure 8/data/group_gene_list/TCGA-"
...
breaks <- c(
  ...
  seq(0, -min_val, length.out = 50 - round(50/2) + 1)[-1]
)
...
top_title <- grid::textGrob(
  "Oncogenic signaling pathways correlated with EOI of glycolysis in tumor samples",
  x = unit(0.5, "npc"),
  y = unit(0, "npc"),   
  just = "center",
...
gt <- gtable::gtable_add_rows(gt, heights = unit(1.3, "lines"), pos = 0)
...
gt <- gtable::gtable_add_grob(
  gt, right_label,
  t = 2, b = nrow(gt), l = ncol(gt)-2, r = ncol(gt)-1,
  clip = "off"
)
p$gtable <- gt
grid::grid.newpage()
...
pdf(paste0(dir,"/figure 8/figures/","figure 8b.pdf"), width = 800/96, height = 600/96)
print(p)
